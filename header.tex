...
%\usepackage[T1, LGR]{fontenc} % LGR caused greek letters in footline
...
\usepackage[breaklinks, hidelinks,hyperfootnotes=false, unicode]{hyperref} % automatische Hyperlinks in Verzeichnissen
...
\usepackage[]{hypcap}  %link to image directly, not to caption


\usepackage{bookmark}


\usepackage{xspace}						% Macht immer ein Leerzeichen, außer Nutzer will mehrere Leerzeichen

%\usepackage{layout}						% Layout überprüfen
\usepackage{setspace} 	% Zeilenabstand singelspacing, onehalfspacing, doublespacing,
...
\newcommand{\showgrid}[2]{%
	\newcommand{\gridlen}{20}  % 5 = 1 cm  % 20 = 4 cm
	\resizebox{#1}{!}{%
...
\usepackage[columns=2,minspace=100pt]{idxlayout} %modify layout of index
